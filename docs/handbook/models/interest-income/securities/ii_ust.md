...
> Shared machinery: three-term template, hedge data states, reinvestment mechanics, and the granularity proposal are stated once in `docs/handbook/cross-cutting/asset-side-common-conventions.md` §12 and cross-referenced here (D-003 pattern).
> Chapter review state: **DRAFTED + source-grounding review this session (`docs/reviews/interest-income/securities/ii_ust.review.md`) — AWAITING USER REVIEW (Increment 2 gate).** Specification: `docs/specifications/interest-income/securities/ii_ust.yaml`.
...
| D-004 boundary (source-stated ÷4 governs); D-005; D-006 | `docs/handbook/open-questions.md`; conventions §4/§12 |
