...
> Shared machinery: three-term template, hedge data states, reinvestment mechanics, floating-margin imputation, and the granularity proposal are stated once in `docs/handbook/cross-cutting/asset-side-common-conventions.md` §12 (D-003 pattern).
> Chapter review state: **DRAFTED + source-grounding review this session (`docs/reviews/interest-income/securities/ii_mbs.review.md`) — AWAITING USER REVIEW (Increment 2 gate).** Specification: `docs/specifications/interest-income/securities/ii_mbs.yaml`.
...
| Unsettled transactions | AC proxy = purchase price/100 × notional when maturity/book yield/AC missing or zero near the settle date | **PID-SEC-3, user-confirmed 2026-07-23** (company convention); **rescoped by PID-SEC-13, 2026-07-28** — the proxy now requires a purchase date within `unsettled_window_days` of the report date (CQSCP095), instead of firing on every blank AC |
| Blank amortized cost (not near-settle) | Config `missing_ac_mode`: `price_proxy_no_accretion` (default, unchanged behavior) \| `price_proxy_accrete` \| `zero_accrete` (blank read as zero and accreted — the reference's own treatment, reproduces its row to −0.0001) | **PID-SEC-13, user-directed 2026-07-28**; rationale and evidence in `docs/handbook/open-questions.md`; owners' confirmation pending as **OQ-031** |
