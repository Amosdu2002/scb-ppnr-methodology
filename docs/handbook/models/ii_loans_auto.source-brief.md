...
| (g) | **wt is a large per-quarter schedule** (amortization-scale, reaching double-digit percent in later quarters), feeding the A38 blend per segment | **RESOLVED 2026-08-13 (user-stated, PID-LOAN-29 as amended): the schedule is SUPPLIED on the pivot sheet — columns P–X = PQ1..PQ9, row 2 = New, row 3 = Used**; upstream derivation stays outside the model boundary (OQ-001 auto leg resolved as a supplied-input contract) |
| (h) | **Balances = the M.1 auto row × the pivot's D_OS shares** (New/Used); the Auto leases row carries zero | Confirms the PID-LOAN-26 wiring for auto; **lease rows confirmed excluded (role-less, zero balance — PID-LOAN-26 as amended 2026-08-13)**; the sheet's pivot-vs-M.1 cross-check cell = a consistency monitor, M.1 stays the multiplicand |
| (i) | **Scalar anomaly: the auto income panel applies × 0.948, not Table A8's 0.865** — the only observed retail block whose scalar differs from the published row (mortgage 1.014, consumer card 0.969, SME 1.033, noncore 1.072 all match) | **RESOLVED 2026-08-13 (user-directed, PID-LOAN-32): production uses the published 0.865** — the panel's 0.948 stands as a recorded compare-basis divergence (expect auto Totals ≈ 0.91 of the reference; config override for reference-matching runs). OQ-010 auto leg resolved for project implementation |
