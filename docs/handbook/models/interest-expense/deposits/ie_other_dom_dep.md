...
> Integrity flags affecting this chapter: SQ-1 (Table A7 Down-row labels are internal parameter names), SQ-2 (Table A7 caption cites "(Equations A46)"), SQ-9 (Eq A45 where-list "indicats"), SQ-12 (two consecutive "A third assumption" labels, PDF p. 214); source quirk **SQ-15** — truncated spread-estimation sentence (PDF p. 212; raised in `docs/reviews/interest-expense/deposits/ie_other_dom_dep.review.md`, filed in `docs/inventory/source-integrity-review.md` §8 at integration, 2026-07-17). December 2025 revision item 5 applies to p. 214 ("end of quarter" → "average"; "FR Y-9C" → "FR Y-14Q"; subsection (a.) → (b); "contradicts" → "abstracts from"; PDF pp. 4–5; md sec-0). Conversion artifacts in v.a(8): none.
> Chapter review state: **REVIEWED** — independent source-grounding review passed 2026-07-17 with no corrections required (`docs/reviews/interest-expense/deposits/ie_other_dom_dep.review.md`). Specification: `docs/specifications/interest-expense/deposits/ie_other_dom_dep.yaml`. Approved content is never silently overwritten.
...
1. **Launch-point extraction (PQ0).** Read rates 42B/42C/42D → Rate(i,b,0); read subcomponent balances from items 34B/34C/34D [PID-ODD-1] (these are both the A47 weights and, summed, the expense multiplicand [PID-ODD-3]); optionally read the monitor reference BHCB3187 + BHOD3187 + BHCB2389 + BHOD2389 [PID-ODD-2, monitor-only per PID-ODD-3]; load Table A7 betas. Validate per §10; no fallback treatment is invented [CODE].
...
- **[PID-ODD-3 — user-confirmed 2026-07-23]** AverageBalance(b) = Σ(items 34B + 34C + 34D) — the same balances that weight Eq A47, so the expense equals Σᵢ Rateᵢ × Balanceᵢ. Confirmed by empirical alignment against the company reference implementation (rates identical; the reference matched the weight-sum expense). The [PID-ODD-2] MDRM sum no longer multiplies the rate; it serves only as the optional §10 consistency-monitor reference.
...
- **[PID-ODD-2 — user-confirmed 2026-07-17]** Total balance mapping: `odd_total_average_balance` = sum of MDRM items **BHCB3187 + BHOD3187 + BHCB2389 + BHOD2389**. Recorded exactly as confirmed; note the source sentence attributes the average balance to "the FR Y-14Q" ([FACT], PDF p. 214) while the confirmed mapping is expressed in MDRM codes — the mapping governs for implementation, the tension is merely noted, not resolved here. Consequence: the A47 weights (34B–34D) and the MDRM sum come from **different physical sources**; their consistency is a §10 monitor, not an identity. **[PID-ODD-3 — user-confirmed 2026-07-23] supersedes the multiplicand role:** the §7 expense multiplies the Σ(34B–34D) weight balances themselves (expense ≡ Σᵢ rateᵢ × balanceᵢ, matching the company reference implementation); the MDRM sum is retained only as the optional §10 monitor reference.
...
- **Input presence (no invented fallbacks; failures surface):** rates 42B/42C/42D; subcomponent balances 34B/34C/34D [PID-ODD-1]; the four MDRM items BHCB3187/BHOD3187/BHCB2389/BHOD2389 [PID-ODD-2] only when the optional monitor reference is supplied [PID-ODD-3]; 2020:Q2–2021:Q4 rate history for spreads (firms without ELB-window history: treatment UNKNOWN — proposed OQ-017).
...
- **Weights:** Balance(i,b) ≥ 0 and ΣBalance > 0 before A47. **Consistency monitor (optional):** the Σ(34B+34C+34D) expense balance vs. the [PID-ODD-2] MDRM-sum reference, when supplied — different physical sources that should be economically close; log divergences, never force equality; the reference never enters the expense [PID-ODD-3].
...
- **OQ-016 — RESOLVED FOR PROJECT IMPLEMENTATION (2026-07-17; revised 2026-07-23 by PID-ODD-3).** Balance mappings user-confirmed as PID-ODD-1 (items 34B/34C/34D — weights and, summed, the expense multiplicand per PID-ODD-3) and PID-ODD-2 (MDRM sum — optional monitor reference only); the source-side absence of named items remains a preserved [FACT]. Both PIDs recorded in the `docs/handbook/open-questions.md` decision-log PID registry (integration, 2026-07-17).
...
| D-004 convention; PID-5 pattern; PID-ODD-1/PID-ODD-2 mappings; PID-ODD-3 multiplicand revision | `docs/handbook/open-questions.md` decision log; `ie_dom_time_dep.md` §12; user confirmations 2026-07-17 (§9 above) and 2026-07-23 (§7) |
