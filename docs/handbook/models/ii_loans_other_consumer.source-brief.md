...
> Deliverable: loans workstream (asset-side Increment 3), retail wave 2 — drafted 2026-08-12 with the mortgage and card briefs at the user's direction. Review state: **APPROVED 2026-08-13** *(user review of the combined retail gate — "Looking good"; banner updated at approval)*.
...
> Physical context: the **M.1 Balances** retail wiring supplies this family's balance side (**PID-LOAN-26**, registered with the mortgage brief; noncore part restated in §0.1); **the rate-side input is NOT YET SUPPLIED** — the workbook counterpart of the "FR Y-14Q pre-provision net revenue line-item report" jump-off rates is this family's outstanding Required input (§0.1). *(Amended 2026-08-13: supplied the next day — the construction and rate source are recorded in §0.2 as PID-LOAN-30, awaiting gate confirmation.)* No firm values appear in this repository.
...
- **(c) The rate-side input.** **Not yet supplied**: the workbook sheet carrying the family's jump-off interest rate(s) — the counterpart of the "firm-level earned interest rates reported in the FR Y-14Q pre-provision net revenue line-item report" (p. 180) — plus the business-line mapping it embodies (OQ-011's physical form). This is the outstanding Required input for the family; the brief's source side is complete without it, the spec stage is blocked on it. *(Superseded 2026-08-13 — supplied the next day; see §0.2 and PID-LOAN-30. Recorded, not silent.)*

### 0.2 Wave-3 construction (2026-08-13) — user-described and screenshot-verified, TO CONFIRM at the gate

**PID-LOAN-30 (registered in `docs/handbook/open-questions.md`):** the user supplied the family's construction ("Here's how they constructed it") and its rate source. As observed — arithmetic-verified on a consumer row — with no firm values entering this repository:

- **Engine: every product row runs the VARIABLE path** — rate(q) = Prime(q) + spread, spread = jump-off earned rate − Prime(PQ0), income(q) = balance × rate(q) ÷ 4 × scalar. **No fixed leg exists** — the OQ-041 project-side answer, observed (and, once confirmed, a recorded divergence candidate against the p. 183 "most non-core loans" fixed-rate prevalence note).
- **Jump-off rates: a PPNR line-item projections sheet** in the FR Y-14A PPNR Projections worksheet layout (average asset balances + average rates earned by line item, PQ0–PQ9; the sheet name references an institution and stays **config-local**, per the PID-LOAN-11 precedent). The construction reads the **PQ0 average-rate of the mapped line** only; the projection path is rebuilt on Prime, not taken from the sheet. **This is the OQ-011 project-side answer, observed** — "most closely aligned business line" realized as an explicit per-product mapping column.
- **Observed line mapping** (product → line-item report row): US Other Consumer (Schedule A.7 sub-products): Secured-Revolving and Unsecured-Revolving → **Credit Cards**; Secured-Installment and Unsecured-Installment → **"Other, incl. loans backed by securities (non-purpose lending)"**; Overdraft → zero rate. US Small Business (Schedule A.9: Line of Credit, Term Loan, Other) → **C&I Loans**. Below-threshold internationals (Schedules A.1/A.3/A.4/A.5/A.6/A.8) → Auto Loans / Credit Cards / HELOCs / First Lien Residential Mortgages / the other-consumer line / C&I Loans respectively; Student (A.10) → **Student Loans**; Non-purpose consumer lending → the non-purpose line.
- **Product grain = FR Y-14Q Retail schedule blocks** (the family's first physical grain statement): A.7 and A.9 supply sub-product **balances** (allocating the M.1 block balance for the domestic blocks); the "Not reported (below thresholds)" internationals and the non-purpose row take **M.1 balances directly** (non-purpose = domestic + international sides combined) — consistent with the family's no-rate-data charter: the retail schedules contribute balances, never rates.
- **Scalars observed: consumer rows × 1.072 ("Noncore"); the small-business block × 1.033** (the merged "C&I, noncore SME loan and card" row) — the §10 three-way question answered in the observed engine: domestic small-business **loans** take 1.033, not 1.072.

**Remaining gate items for this family:** confirm the mapping table and the M.1-rows-per-block wiring (§0.1 (a)/(b) fold into this); confirm the no-fixed-leg reading; the lease rows' membership; and the schedule-block ↔ results-sheet regrouping (the results summary re-groups the family as Students / Small business / International mortgages / All other consumer).
