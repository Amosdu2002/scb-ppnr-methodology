...
| **PID-LOAN-13 … PID-LOAN-17** (2026-08-12) | **The reference-engine set — the company workbook's own construction, CONVERGED against the reference (grand 0.9979)**: utilized share basis ("Launchpoint Outstanding Balance" = Σ Utilized Exposure Global per segment — a derived quantity, not a column); balances = M.1(LOCOM side) × utilized-share; variable = floating+mixed merged at the floating spread (supersedes PID-LOAN-4's mixed hybrid in this engine); zeros-included utilized-weighted floor, max 0; fixed floored at 0; block Total = (Fixed+Variable) × Table A8; merged bucket = M.1(9+10+11) at the depository floating+mixed rate. Full text in `docs/handbook/open-questions.md`; construction in the spec §9a | Company-workbook conventions, never attributable to the Federal Reserve; `engine="pid"` retains the originally registered construction |
| **PID-LOAN-2 … PID-LOAN-8** (2026-08-03) | **The Corporate implementation set** — segment key and granularity; rate pools and initial interest rates; spread construction by rate type; engine assignment with rate types `0`/`4` counting toward balance but earning nothing; wt from contractual maturities; floors from CORP H.1; and the input contract. Full text in `docs/handbook/open-questions.md`; the computation is specified in **`docs/specifications/interest-income/loans/ii_loans_corporate.spec.md`** | Mixed: PID-LOAN-6 (wt) and PID-LOAN-5 (fee-based balances in the denominator) are **recorded divergences** from the Fed text; the others operationalize rules the source states without mechanics. Resolve **OQ-001, OQ-002, OQ-003** for Corporate project implementation; all three source-side gaps preserved |
