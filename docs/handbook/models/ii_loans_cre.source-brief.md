...
| **PID-LOAN-22** | The Eq A37 origination-date statistic is **outstanding-balance-weighted** (sheet-stated: "balance weighted orig date based on: outstanding") — unlike Corporate's PID-LOAN-4 unweighted row median. *(Amended 2026-08-12, first compare: the statistic is the weighted **median** — the reference's median-date cells are observed dates, and the mean missed construction fixed by one quarter, ratio 1.0447.)* | The Fed states only "the median origination date … for that portfolio" (PDF p. 182); **resolves the OQ-003 CRE leg for project implementation**; source-side gap preserved |
| **PID-LOAN-23** | Mixed facilities project on the variable engine. *(Amended 2026-08-12, first compare: at the **floating** spread — the reference's implied variable spread equalled the v2 spread exactly, so the launch sheet's hybrid mixed-spread columns are computed but **unused**, precisely the pattern by which PID-LOAN-15 superseded PID-LOAN-4 for Corporate.)* **Engine constructions are per wholesale part** | The Fed states only that mixed-rate loans are "treated as variable-rate" (PDF p. 176, Corporate-located); CRE applicability of that rule itself is **OQ-035** |
...
| **OQ-003** | CRE leg **RESOLVED FOR PROJECT IMPLEMENTATION 2026-08-12 (PID-LOAN-22 as amended: the outstanding-weighted median, compare-identified)**; source side preserved | Differs from Corporate's PID-LOAN-4 unweighted median; the wholesale parts legitimately diverge (§5 row 8) |
