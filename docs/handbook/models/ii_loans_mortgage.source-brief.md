...
| (j) | **Missing-window fallback observed**: a fixed segment with "x" in both `AFTER` windows takes its **own current weighted rate** as the new-origination rate | §0.1 (d) sharpened into a stated rule — **ADOPTED as a flagged working assumption 2026-08-13 (PID-LOAN-33 as amended): the user reports the `AFTER` columns are hardcoded query output, so no formula lineage exists; the fallback (segment's own current rate as the new-origination rate) mirrors the observed build-time convention, runs config-visible with a per-segment census, and the compare arbitrates** |
