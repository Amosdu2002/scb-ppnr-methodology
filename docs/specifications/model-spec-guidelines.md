...
specification under `docs/specifications/`; the five liability-side interest-expense specs were
...
- Path: `docs/specifications/<side>/<subfamily>/<model_id>.yaml`, mirroring
  `docs/handbook/models/<side>/<subfamily>/<model_id>.md` (e.g.
  `docs/specifications/interest-expense/deposits/ie_dom_time_dep.yaml`).
...
- `PID` — user-confirmed project implementation decision (registry:
  `docs/handbook/open-questions.md`); never attributable to the Federal Reserve.
...
`docs/handbook/cross-cutting/liability-side-common-conventions.md`, applied per model):
...
The single registry for both lives in `docs/handbook/open-questions.md`.
