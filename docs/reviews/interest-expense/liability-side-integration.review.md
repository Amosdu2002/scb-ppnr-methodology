...
`ie_other_borrowing` — across `docs/handbook/models/interest-expense/`,
`docs/specifications/interest-expense/`, `docs/reviews/interest-expense/`, and the shared tracking files.
...
- **F-1 — `ie_dom_time_dep` had no YAML specification** anywhere in the repository.
  *Resolved:* `docs/specifications/interest-expense/deposits/ie_dom_time_dep.yaml` created at this
...
- **F-3 — Canonical model-ID mismatch.** `docs/inventory/model-inventory.md` (record #10 and the
  dependency summary) and `docs/inventory/source-map.md` used `ie_ffp_repo`; all merged artifacts use
...
- **F-5 — Decision-log gaps.** Thirteen user-confirmed PIDs (PID-1…PID-6, PID-ODD-1/2,
  ...
  PID registry table added to the `docs/handbook/open-questions.md` decision log.
...
`docs/specifications/model-spec-guidelines.md` created (shared key order, `model` metadata incl.
...
- **`ie_dom_time_dep`** — a focused independent source-grounding review to produce the missing
  report artifact at `docs/reviews/interest-expense/deposits/ie_dom_time_dep.review.md` (F-2), which
...
the shared conventions (`docs/handbook/cross-cutting/liability-side-common-conventions.md`) and the
matrix (`docs/inventory/liability-side-model-matrix.md`), treating §5's items as configuration
...
plus the FRB total path feed the OB calibration (registry: `docs/handbook/open-questions.md`; chapter
