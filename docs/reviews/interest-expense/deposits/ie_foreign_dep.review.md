...
Files under review: `docs/handbook/models/interest-expense/deposits/ie_foreign_dep.md`, `docs/specifications/interest-expense/deposits/ie_foreign_dep.yaml`.

## 1. Method

Every material claim in the chapter and YAML was traced to a PDF page image inspected this session. The v.a(8) framework pages (211–214) were verified in full because v.a(9) incorporates Equations A45–A47 by reference; Table A7 (p. 219) and the 44B usage in v.a(10) (p. 217) were also verified. The Markdown working copy was found to match the PDF at every checked point, consistent with `docs/inventory/source-integrity-review.md`.
...
5. **`docs/handbook/open-questions.md` OQ-013**: extend the entry to note it now affects two chapters (`ie_other_dom_dep`, `ie_foreign_dep`) and record the working branch choice (= 25 bp → non-ELB) if the user confirms it as a PID.
