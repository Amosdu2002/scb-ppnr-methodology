...
**Review date: 2026-07-23.** Reviewer: this session, against the authoritative PDF page images (pp. 188–190 opened and read in full this session; structural-intro and Table A6 claims cross-checked against md sec-148/sec-149 and the verified `docs/inventory/source-integrity-review.md`).
Files under review: `docs/handbook/models/interest-income/calculators/ii_dep_banks_other.md`, `docs/specifications/interest-income/calculators/ii_dep_banks_other.yaml`.
...
- **MINOR-2:** Chapter numbering ("# 2.") follows the inventory census (#2); the asset-side chapter-number sequence is recorded in `docs/inventory/asset-side-model-matrix.md`. No action.

## 3. Shared-file changes (applied this session — Increment 1)

1. **`docs/inventory/asset-side-model-matrix.md`**: row for `ii_dep_banks_other` — DRAFTED, awaiting user review.
2. **`docs/handbook/open-questions.md`**: D-009 (compact chapter skeleton) recorded; no new OQ from this chapter (SQ-3/SQ-4 already filed 2026-07-16).
3. **`docs/inventory/source-map.md`** and inventory #2 artifact row: deferred to the asset-side integration gate, mirroring the liability-side practice.
