...
**Every model below is PROPOSED for the 2026 stress test (public-comment stage) — NOT adopted.** Source: Section B.v of the Fed PPNR model documentation (October 2025, updated December 2025). Citations are `(PDF p. N; md sec-M)`; page conventions per `docs/inventory/source-integrity-review.md`.
...
| Artifacts (loans slices 1–3 + retail R1) | Source briefs: `docs/handbook/models/ii_loans_common.source-brief.md` (owns Eqs A32–A38 verbatim + the common-boundary register over PDF pp. 173–188) and `docs/handbook/models/ii_loans_wholesale.source-brief.md` (Corporate∩CRE shared framework; cites equations, transcribes none) — **APPROVED 2026-08-03**; `docs/handbook/models/ii_loans_corporate.source-brief.md` (11-portfolio census, Corporate segmentation grid, rate-type mechanics, NPML proxy exception, fee-only dual exclusion) — **APPROVED 2026-08-12** (user statement; banner updated same day, amendment recorded); `docs/handbook/models/ii_loans_cre.source-brief.md` (6-type census, 24-segment grid, inheritance register, fact-of-absence register, OQ-039, PID-LOAN-18..25) — **APPROVED 2026-08-12**. Computation specs: `docs/specifications/interest-income/loans/ii_loans_corporate.spec.md` (converged vs the company reference 2026-08-12) and `ii_loans_cre.spec.md` (created 2026-08-12 with the engine extension — `build_cre_launch_point`, H.2 loader/mapping, CRE compare mode; **reference compare CONVERGED 2026-08-12** — round 1 grand 1.0008, round 2 grand 1.0006 after the PID-LOAN-22/23 amendments; the round-3 grand line under the corrected config is owed for the spec record). **Retail briefs ALL APPROVED 2026-08-13 (combined gate):** `docs/handbook/models/ii_loans_retail.source-brief.md` (retail framework — four-family census, retail-boundary register over sec-156–160 + sec-170, Prime-except-mortgages base-rate application, Eq A36 retail-branch application, Table A8 retail-row census; SQ-23/SQ-24, OQ-040/OQ-041 filed) and `docs/handbook/models/ii_loans_auto.source-brief.md` (auto family — A.2 segment-level basis, all-fixed assumption, HFS→HFI reclass, new/used grid, Prime spread benchmark, elicitation register; OQ-042 filed); **Retail wave 2 (same day, approved at the same gate):** `ii_loans_mortgage.source-brief.md` (12-segment physical grid vs the Fed 2×2, classification-variant question, A36 windows, ARM floors, maturity-only wt risk; PID-LOAN-27), `ii_loans_card.source-brief.md` (two sub-portfolios, revolver machinery, income arrangement [INT], SQ-25 + OQ-043 filed, SME scalar question; PID-LOAN-28), `ii_loans_other_consumer.source-brief.md` (census + engine tension OQ-041, jump-off OQ-011, M.1 noncore wiring; **rate-side input still outstanding**). **Wave 3 (2026-08-13): the user supplied the remaining inputs and the full calculation-sheet layouts** — auto pivot contract, other-consumer construction + rate source, MEV columns, results targets (PID-LOAN-29/30/31; §0.2 addenda in all five briefs). **The retail input set is COMPLETE**; still to come: the combined retail gate + the §0.2 confirmation list (highlights: MORT-vs-alternative sheet selection; the auto ×0.948-vs-0.865 scalar; the card 12-month-activity condition), per-family specs/engines/compares, and the Wholesale integration review; no chapter exists yet (chapter granularity unchanged: one chapter, six portfolio sections per D-003) |
...
| Artifacts (integration 2026-07-17) | Chapter `docs/handbook/models/interest-expense/deposits/ie_dom_time_dep.md` — REVIEWED (review report artifact not preserved — follow-up focused review required); spec `docs/specifications/interest-expense/deposits/ie_dom_time_dep.yaml` (created at integration); brief `docs/handbook/models/interest-expense/deposits/ie_dom_time_dep.source-brief.md` |
...
| Artifacts (integration 2026-07-17) | Chapter `docs/handbook/models/interest-expense/deposits/ie_other_dom_dep.md` — REVIEWED; spec `docs/specifications/interest-expense/deposits/ie_other_dom_dep.yaml`; review `docs/reviews/interest-expense/deposits/ie_other_dom_dep.review.md` (PASS, no corrections) |
...
| Artifacts (integration 2026-07-17) | Chapter `docs/handbook/models/interest-expense/deposits/ie_foreign_dep.md` — REVIEWED; spec `docs/specifications/interest-expense/deposits/ie_foreign_dep.yaml`; review `docs/reviews/interest-expense/deposits/ie_foreign_dep.review.md` (source-faithful) |
...
| Artifacts (integration 2026-07-17) | Chapter `docs/handbook/models/interest-expense/funding/ie_fed_funds_repo.md` — REVIEWED; spec `docs/specifications/interest-expense/funding/ie_fed_funds_repo.yaml`; review `docs/reviews/interest-expense/funding/ie_fed_funds_repo.review.md` (APPROVE) |
...
| Projection to dollars | **RESOLVED FOR PROJECT IMPLEMENTATION 2026-08-13 (PID-TRD-2/PID-TRD-3, reference-observed):** dollars = (β·T3m(q) + α_b) × NetTA(b,0) **÷ 4** — the reference projects an **annualized** rate with the ÷4 at the dollar step (arithmetic verified in-session on the trading-tab screenshot); NetTA(b,0) = Schedule G sheet cells **R30 − R112** (panel labels 13/37 read as the worksheet line items [INT]), flat. The source states **no projection mechanics at all** (pp. 225–230 image-verified) — OQ-007 resolved-for-project, source-side absence preserved; the source-literal quarterly-LHS reading (which would imply no ÷4 and 4× the scenario sensitivity) is retained as recorded tension — chapter §5 |
| Fixed-effect values | **PROJECT IMPLEMENTATION DECISION (PID-TRD-1, user-confirmed 2026-08-13; supersedes D-002, which now has no remaining scope; basis amended same day by PID-TRD-3):** α_b calibrated in closed form so the nine-quarter cumulative net trading NII equals the residual implied by `frb_total_interest_income` minus the six sibling income models — the PID-OB-5 income-side mirror, **PID-OB-5 closed-form shape with ×4** (annualized basis per PID-TRD-3); PQ0 actuals never used; α_b an annualized intercept (observed on the tab's Coefficients column against a constant-1 regressor row); requires the six sibling income paths (project-level execution order) — resolves OQ-009 for this model |
...
| Open questions | OQ-005, OQ-023 (extended 2026-08-13) open; **OQ-007 RESOLVED FOR PROJECT IMPLEMENTATION (PID-TRD-2/3, 2026-08-13)** and **OQ-009 RESOLVED FOR PROJECT IMPLEMENTATION (PID-TRD-1, 2026-08-13)** — source-side absences preserved |
| Project implementation decisions | **PID-TRD-1** (2026-08-13): nine-quarter cumulative α_b calibration vs the FRB total-interest-income residual; **PID-TRD-2** (same day): physical cells R30/R112 on sheet "14Q Sch G" + β via FRB-provided coefficients (= published 0.278); **PID-TRD-3** (same day): annualized-÷4 basis, closed form = PID-OB-5 shape ×4, quarterly-LHS tension recorded (full text in `docs/handbook/open-questions.md`). Remaining elicitation: the "Implied FRB results" row's formula references (Helpful — subtraction basis per PID-SEC-8/PID-LOAN-32); the two reference rows PQ1..PQ9 at compare |
| Artifacts (Increment 4) | Chapter `docs/handbook/models/interest-income/trading/nii_trading_al.md` (first asset-side regression chapter — D-009 skeleton + the two `ie_other_borrowing` sections) — **REVIEWED 2026-08-13, user gate pending**; spec `docs/specifications/interest-income/trading/nii_trading_al.yaml`; review `docs/reviews/interest-income/trading/nii_trading_al.review.md` |
...
| Artifacts (integration 2026-07-17) | Chapter `docs/handbook/models/interest-expense/funding/ie_other_borrowing.md` — REVIEWED; spec `docs/specifications/interest-expense/funding/ie_other_borrowing.yaml`; review `docs/reviews/interest-expense/funding/ie_other_borrowing.review.md` (APPROVE WITH OPEN IMPLEMENTATION ITEM) |
