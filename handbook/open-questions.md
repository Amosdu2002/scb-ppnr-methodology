...
**Status: OPEN (working method fixed by D-002).** Table A9: "Estimated coefficients for firm fixed-effects are not included in the table" (PDF p. 234; md sec-224); "backsolve" appears nowhere in the source. Working method (INTERPRETATION): recover α_b at the launch point from observed actuals given disclosed macro coefficients. Undefined: which quarter(s) of actuals (single launch-point quarter vs. an average), data source for the actuals, and treatment if the ELB/sample regime differs from the estimation window (A53 was estimated on 2020:Q2–2021:Q4). Resolves: Fed disclosure of fixed effects, or a validated backsolve specification in the coding phase (generic, non-confidential).
