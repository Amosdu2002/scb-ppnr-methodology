...
### 0.2 Wave-3 framework-level contracts (2026-08-13) — user-supplied, registered as PID-LOAN-31

- **MEV contract:** the scenario input is the **"MEV Data"** sheet — the Federal Reserve scenario-table layout (Scenario Name; Date; one column per MEV), with column headers observed and user-supplied: **"Prime rate"**, **"Mortgage rate"**, **"3-month Treasury rate"** (alongside the 5-/10-year Treasury, BBB, and macro columns), **percent** units; rows = "Actual" history from 1976 Q1 plus the "Supervisory Severely Adverse" projection block. This **confirms the two retail scenario-series columns that were TO_BE_CONFIRMED** in the config template (§5.1) and matches the existing MEV loader contract (history block = "Actual"; projection rows mapped to PQx by date).
- **Multi-workbook configuration (user-directed):** the retail inputs span **multiple workbooks** ("note all of them coming from different workbook, so we need to enter those in the config") — the auto pivot and Y-14Q Retail A.2/A.7/A.9 extracts live in a separate file from the main workbook's M.1 / query / calc / MEV sheets. [CODE] the `[firm_data.loans]` config gains per-sheet workbook paths (the securities per-block-path precedent).
- **Results contract (compare targets):** the user supplied "the eventual result": a summary table of per-family **4Q-cum and 9Q-cum income under the FRB model vs the firm's champion-model results** (sub-product rows per family; the champion label references an institution — config-local), plus per-family income panels on the calc sheets ({Fixed Income, Variable Rate Income, Total × scalar} paths per block — the wholesale block-Totals pattern). These are the retail compare-mode targets.
