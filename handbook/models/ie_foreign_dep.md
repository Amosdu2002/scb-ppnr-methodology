...
> Chapter review state: **DRAFT** — see `ie_foreign_dep.review.md` for the independent PDF-grounding review.
...
|---|---|---|---|---|---|
| Foreign deposits rate (non-time), Rate(nontime,b,0) (`foreign_nontime_rate_liftoff`) | 43A ("foreign deposits") | b | Annualized rate [PID-6/D-004] | Lift-off (q0); seeds the A46 recursion [INT-c] | [FACT] item (PDF p. 215; md sec-202); [INT] seed role |
| Foreign deposits–time rate, Rate(time,b,0) (`foreign_time_rate_liftoff`) | 44B ("foreign deposits-time") | b | Annualized rate | Lift-off (q0); seed [INT-c] | [FACT] item (PDF p. 215); [INT] seed role; quirk-candidate: 44B also names the repo *balance* item in v.a(10) (PDF p. 217) — see §12 |
| Foreign non-time balance, Balance(nontime,b) (`foreign_nontime_balance`) | 35A | b | USD | Lift-off; held constant [INT-d] | [FACT] item (PDF p. 215) |
| Foreign time balance, Balance(time,b) (`foreign_time_balance`) | 35B | b | USD | Lift-off; held constant [INT-d] | [FACT] item (PDF p. 215) |
...
| Firm-reported betas (`foreign_beta_reported`) | 83A, 83B, 84A, 84B | b × i × {up,down} | Fraction | Lift-off; **feed the Table A7 medians only** — model uses medians | [FACT] items (PDF p. 215); item→(subcomponent, direction) mapping unstated [INT-b] |

### 3.2 Scenario inputs

| Variable | Enters via | Frequency | Units | Label |
|---|---|---|---|---|
| 3-month Treasury yield, Treasury3m(t) (`usd_3m_treasury`) | Regime test (25 bp); level in A45; change ΔTreasury3m in A46; First_ELB_Treasury3m for the assumed floor | Quarterly, t = 1…9 (plus q0 value for ΔTreasury3m at t = 1 [INT-f]) | Annualized rate | [FACT] (PDF pp. 211–213, 216); MEV column name per PID-5 pattern presumed "USD 3M Treasury" — exact name **UNCONFIRMED** [CODE] |
...
| Quantity | Lift-off (q0) role | Projection (t ≥ 1) role | Label |
|---|---|---|---|
...
| Treasury3m(t) | q0 value used only for ΔTreasury3m at t = 1 [INT-f] | Contemporaneous each quarter; drives regime, level, change | [FACT] |
...
[INT-d] Balance(i,b,t) carries a t subscript in the source; under the constant-balance convention (PDF pp. 169–170, 218; md sec-148, sec-206) Balance(i,b,t) = Balance(i,b,q0) — items 35A/35B at lift-off.

## 6. Calculation workflow

1. **Lift-off extraction.** Read rates 43A/44B → Rate(i,b,0) [INT-c], balances 35A/35B → Balance(i,b) [FACT items]. Validate per §10.
...
4. **Scenario prep.** Align Treasury3m to q0…PQ9; ΔTreasury3m(t) = Treasury3m(t) − Treasury3m(t−1) [INT-f: Δ undefined in source beyond "the change"]; First_ELB_Treasury3m from the scenario path [FACT].
...
- [INT-a] Working assumption: expense balance = Balance(nontime,b) + Balance(time,b) = items 35A + 35B at lift-off — the same balances the source names for this component.
...
- Missing/negative lift-off rate (43A, 44B) or balance (35A, 35B); both balances zero → A47 divides by zero.
...
**Working-assumption register (flagged INTERPRETATIONS used throughout):** **INT-a** expense balance = items 35A + 35B (§7); **INT-b** beta items 83A/83B → non-time up/down, 84A/84B → time up/down, by ordering parallel with 79A–81B (§3.1); **INT-c** recursion seeds Rate(i,b,0) = lift-off items 43A/44B (§3.1); **INT-d** Balance(i,b,t) = Balance(i,b,q0) via the general constant-balance convention (§5); **INT-f** ΔTreasury3m(t) = Treasury3m(t) − Treasury3m(t−1), with the q0 value seeding t = 1 (§6). Each is a candidate PID for user confirmation (§9).
