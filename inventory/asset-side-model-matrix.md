...
| 2 | B (securities) | `ii_ust`, `ii_mbs`, `ii_other_sec` | **Handbook DRAFTED 2026-07-23 — AWAITING USER REVIEW.** OQ-004 RESOLVED (Securities Model Description collected; reinvestment + vendor passages extracted, image-verified; conventions §12 added). **Gate decisions for the user:** PID-SEC-1 (pre-aggregated bucket inputs), PID-MBS-1 (Agency RMBS vendor income as declared quarterly path), OQ-025 working treatments (reinvestment roll/attribution; `usd_1y_treasury` mapping). New OQs filed: OQ-025/OQ-026/OQ-027; SQ-8 refined + CA-4 filed |
