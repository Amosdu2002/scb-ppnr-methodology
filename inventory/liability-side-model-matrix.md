...
nine projection quarters; no upstream model dependencies; validation principles — are stated once
...
| `ie_other_borrowing` | Annualized; ÷4 at final step only (D-004; backsolve reverses the same ÷4) | Exposes expense path; v.c external; OQ-005; **α_b embeds hedge intensity** (Fed rationale) — double-counting caution if v.c is later applied | OQ-005, OQ-009 (narrowed), OQ-022; PID-OB-4 BBB input gates (4 items); PQ0 actual-expense line item | `handbook/models/interest-expense/funding/ie_other_borrowing.md` | `specifications/interest-expense/funding/ie_other_borrowing.yaml` | `reviews/interest-expense/funding/ie_other_borrowing.review.md` — APPROVE WITH OPEN IMPLEMENTATION ITEM |
...
- Only `ie_other_borrowing` has estimated coefficients and a firm fixed effect (backsolved per
  D-002/PID-OB-1/PID-OB-3); the four structural models have none.
- Expense-balance sourcing differs deliberately: `ie_other_dom_dep`'s A47 weights and expense
  ...
  consistency monitor); `ie_foreign_dep` uses the same 35A/35B balances for both (INT-a).
